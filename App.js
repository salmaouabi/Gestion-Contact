    ...
    <ul>
      {contacts.map((contact, index) => (
        <li key={index}>
          ...
          <div>
            <button onClick={() => onDeleteContact(index)}>Supprimer</button>
            <Link to={`/contacts/edit/${index}`}>
...
const App = () => {
  ...
  const updateContact = (index, updatedContact) => {
    const updatedContacts = contacts.map((contact, i) =>
      i === parseInt(index) ? updatedContact : contact
    );
    ...
    <Router>
      ...
      <Routes>
        ...
        <Route
          ...
          element={<EditContact contacts={contacts} onUpdateContact={updateContact} />}
        />
        <Route
          path="/contacts/:id"
